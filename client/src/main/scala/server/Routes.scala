...
trait Routes {
  ...
  lazy val httpRoutes: Route = {
    path("weather") {
      get {
        /*val temp: Future[Weather] = (weatherActor ? GetWeather).mapTo[Weather]
        //complete(temp)
        onSuccess(weatherActor ? GetWeather) { value =>
          //complete requires Marshaller class --> String already marshalled
